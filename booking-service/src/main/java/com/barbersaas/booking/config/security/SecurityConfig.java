        ...
        .authorizeHttpRequests(
            authorization ->
                authorization
                    ...
                    .access(
                        new WebExpressionAuthorizationManager(
                            "(hasRole('CUSTOMER') or hasRole('ADMIN')) "
                                + "and hasAuthority('SCOPE_bookings.write')"))
                    .requestMatchers(org.springframework.http.HttpMethod.GET, "/api/v1/bookings/**")
                    .access(
                        new WebExpressionAuthorizationManager(
                            "(hasRole('CUSTOMER') or hasRole('BARBER') or hasRole('ADMIN')) "
                                + "and hasAuthority('SCOPE_bookings.read')"))
